...
def train():
    ...
    config = {
        ...
        "resume_projectors_from": "./model/trained5_15e_projectors_stage1_train_name_new_prompt/projectors_epoch1_last.pt",
    ...
    config["save_dir"] = f"./model/trained5_15e_projectors_{config['current_stage']}_train_name_new_prompt"
    # 固定随机种子，增强可复现性
    ...
    def text_metrics_once(model, loader, tokenizer, max_batches=8):
        ...
        for batch in loader:
            if batch is None:
                continue
            struct = batch["struct_embeds"].to(device).to(dtype=dec_dtype)
            seq    = batch["seq_embeds"].to(device).to(dtype=dec_dtype)
